...
I hope you see where I'm going here. If they are truly scalars, then both of the above commutation relations would be zero, because multiplication is commutative. But no, something is not being stated... They can't possible be scalars, and so what is the meaning of Newton's equation above?
...
$$ \hat{\bf p}\ \psi = \frac{\hbar}{i}\frac{\partial}{\partial x} \psi $$


For now, we will assume the form above for all wave functions, but you can see the notes at the end of the article for more details on a general case (instead of just a plane wave).
